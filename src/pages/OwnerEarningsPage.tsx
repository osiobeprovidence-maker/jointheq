...
export default function OwnerEarningsPage() {
   ...
   // Memoize and deduplicate listings
   ...
   const stats = {
      totalRevenue: memoizedListings?.reduce((acc, l) => acc + (l.status === 'Active' ? (l.owner_payout_amount || 0) : 0), 0) || 0,
      activeListings: memoizedListings?.filter(l => l.status === 'Active').length || 0,
      pendingVerifications: memoizedListings?.filter(l => l.status === 'Pending Review').length || 0,
   };
                     ...
                     {memoizedListings.map((l) => (
                        ...
                        >
                           <div className="flex items-center gap-6">
                              <div className={`w-16 h-16 rounded-3xl flex items-center justify-center font-black text-white ${l.platform.includes('Netflix') ? 'bg-red-500' :
