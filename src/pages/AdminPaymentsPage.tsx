...
export default function AdminPaymentsPage() {
  ...
  const handleApprove = async (id: any) => {
    if (!confirm("Are you sure you want to approve this payment? This will credit the user's wallet.")) return;
    try {
      setProcessingId(id);
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white p-8 rounded-[2rem] border border-black/5 shadow-sm">
            ...
            <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest mt-1">Pending Requests</div>
          </div>
          <div className="bg-white p-8 rounded-[2rem] border border-black/5 shadow-sm">
            ...
            <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest mt-1">Approved Today</div>
          </div>
          <div className="bg-white p-8 rounded-[2rem] border border-black/5 shadow-sm">
            <div className="w-10 h-10 bg-black/5 text-zinc-400 rounded-2xl flex items-center justify-center mb-4">
              <Wallet size={20} />
            </div>
            <div className="text-3xl font-black">₦{requests?.reduce((acc, r) => acc + (r.status === 'Approved' ? r.amount : 0), 0).toLocaleString()}</div>
            <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest mt-1">Total Volume Verified</div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {!requests ? (
            ...
            <div className="bg-white border border-black/5 rounded-[2rem] p-20 text-center space-y-4">
              ...
              <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Queue Clear</p>
            </div>
          ) : (
                ...
                {requests.map((req) => (
                  ...
                  >
                    ...
                    <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-8 w-full lg:w-auto">
                      ...
                      <div className="space-y-1.5">
                        <p className="text-[10px] uppercase font-black text-zinc-300 tracking-widest">Requested on</p>
                        <p className="font-bold text-zinc-600 flex items-center gap-2 text-sm">
                          <Calendar size={14} className="text-zinc-300" /> {new Date(req.created_at).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="space-y-1.5">
                        <p className="text-[10px] uppercase font-black text-zinc-300 tracking-widest">User ID</p>
                        <p className="font-bold text-zinc-400 text-[10px] break-all uppercase tracking-tighter">{req.user_id}</p>
                      </div>
                    </div>
